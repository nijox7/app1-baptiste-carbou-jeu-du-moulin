...
while True:
    if joueur == "red":
        if retire:
            ...
            trouve = False
            c_vides = cases_vides(plateau)
            for case_v in c_vides:
                case_v = joueur
                if detect_moulin(case_v) != []:
                    case_v.retire()
                    case = case_v
                    trouve = True
                    break
            ...
            trouve = False
            dep_vois = depart.voisines
            for voisine in dep_vois:
                if voisine.couleur == "":
                    voisine.place(joueur)
                    depart.retire()
                    if detect_moulin(voisine) != []:
                        depart.place(joueur)
                        voisine.retire()
                        case = voisine
                        trouve = True
    ...
    elif tev == "ClicGauche" or joueur == "red":
        if joueur != "red":
